...
// 倍率仅在明显偏高时用警示色，常规倍率保持中性（简约基调）
function getRatioBadgeClassName(ratio: ApiKeyGroupOption['ratio']) {
  if (typeof ratio !== 'number') return ''
  if (ratio > 5) return 'border-destructive/30 bg-destructive/10 text-destructive'
  if (ratio > 3) return 'border-warning/30 bg-warning/10 text-warning'
  return ''
}
