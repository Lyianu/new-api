...
func InitOptionMap() {
	...
	common.OptionMap["GroupModelLimit"] = setting.GroupModelLimits2JSONString()
	...
	// DefaultCollapseSidebar 仅 classic 主题消费；default 主题侧边栏固定展开，设置入口已移除
	...
	common.OptionMap["UserVisibleChannel"] = strconv.FormatBool(operation_setting.UserVisibleChannel)
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "UserVisibleChannel":
			operation_setting.UserVisibleChannel = boolValue
	...
	case "GroupModelLimit":
		err = setting.UpdateGroupModelLimitsByJSONString(value)
	case "RetryTimes":
